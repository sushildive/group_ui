...
//            this.$closest = this.$element.closest(this.options["closest"]);
//        }
    ...
    Modal.prototype = {
        ...
        , show: function () {
            ...
            backdrop.call(this, function () {
                ...
                transition ?
                    that.$element.one($.support.transition.end, function () { that.$element.trigger('shown') } ) :  that.$element.trigger('shown');
        ...
        , hide: function ( e ) {
            ...
            $.support.transition && this.$element.hasClass('fade') ?
                hideWithTransition.call(this) : hideModal.call(this);
        }
...
;(function ( $, context, document, undefined ) {
    ...
    Plugin.prototype = {
        ...
        setupEvents: function() {
            ...
            this.$element.find("." + this.options.headingClass ).hover(
                function () {
                    $(this).addClass("hover");
                },
                function () {
    ...
    // Expose object for test purposes
        ...
        init: function() {
            ...
            this.$element.on( "submit", function() {
                ...
                if ( openPopup ) {
                    ...
                    data.push({ name: "requestedResultType", value: "ajax"});
    ...
    Plugin.prototype = {
        ...
        submitHandler: function() {
            ...
            $.ajax({
                ...
                dataType: "json",
                beforeSend: function() {
                    if ( self.$element.hasClass("ajax-stuck"))
                        SpeedboatUI.ModalSingleton.getInstance().showStuckLoading();
                    else
...
//                },
        ...
        unsuccessfulPost: function( json ) {
            // Show message and then dismiss popup
            if (json.message ) {
...
;(function ( $, context, document, undefined ) {
    ...
    Product.prototype = {
        ...
        reset: function() {
            ...
            if (!this.$el.find('.quantity').is("input")) {
                //hard-code value to total price
        ...
        render: function() {
            ...
            this.$el.trigger("productUpdated", { configured: this.checkAllConfigurationOptionsSet(self.options.configurationRadioSelector) });
        },
    ...
    // ***************************************************
    // Convenience Object to manage state of basket button
    // ***************************************************
...
;(function ( $, context, document, undefined ) {
    ...
    var pluginName = 'CatalogueNavigation',
        defaults = {
            ...
            catalogueProductsSelector: "#catalogue",
    ...
    Plugin.prototype = {
        ...
        init: function() {
            ...
            // Store references to placeholder divs
...
//            } );
            ...
            this.$element.on("click", "." + this.options.catalogueItemClass + " a", function() {
                var ProductCatalogueItem = $(this);
                $(ProductCatalogueJson.categories).each (function(i, categories) {
                    if ( categories.name.toLowerCase() == ProductCatalogueItem.attr ("category-name") )
                        $(categories.subcategories).each(function(j, subcategory){
                            if ( subcategory.name.toLowerCase() == ProductCatalogueItem.attr ("subcategory-name") ) {
                                self.clickHandler.call( this, self, ProductCatalogueItem );
//                                $(self.options.catalogueProductsSelector).CatalogueProducts( subcategories.products );
                                $(self.options.catalogueProductsSelector).empty();
                                $(subcategory.products).each( function(j, product) {
                                    $(self.options.catalogueProductsSelector).append ( $("<div></div>").html(product.name));
                                })
                            }
                        });
                });
        ...
        clickHandler: function( context, currentItem ) {
...
;(function ( $, context, document, undefined ) {
    ...
    var pluginName = 'CatalogueProducts',
    ...
    Plugin.prototype = {
        ...
        init: function() {
            alert ("start build catalogue products")
        }
...
(function ( $, context, document, undefined ) {
    ...
    Plugin.prototype = {
        ...
        setFilterDefaults : function(){
            ...
            this.$element.find('#startDatePicker').attr('data-default-value',Utils.returnStandardFormattedDate_ddmmyyyy(startDate));
        ...
        fixFilterFieldWidthsIE7 : function(){
            ...
            //$(".searchFilterFieldGrouping>div.filterField").width(widest);
        },
        ...
        setupEventHandlers: function() {
            this.pageLoad();
            //this.$element.bind('submit', $.proxy( this.disableFormOnSubmit, this ));

            $('#disableFiltersCheckbox').bind('click change', $.proxy( this.disableFiltersCheckboxClick, this ));

            $("#startDatePicker").ready($.proxy( this.loadDatePicker, this));

        },

        loadDatePicker: function(e) {
            if(e.datepicker != undefined){
                $("#startDatePicker").datepicker(e.datepicker.regional[$.cookie("locale",{ path: '/' })], { dateFormat: 'dd/mm/yy' });
                $("#endDatePicker").datepicker(e.datepicker.regional[$.cookie("locale",{ path: '/' })], { dateFormat: 'dd/mm/yy' });
            }
        ...
        setupDateRangeValidationHandler: function(){
            if($('#startDatePicker') && $('#endDatePicker')) {
                $('#startDatePicker').change(this.validateStartEndDates);
        ...
        validateStartEndDates: function(e) {
            ...
            if (startDate != "" && endDate != "") {
                var isSelectingStartDate    = $('#startDatePicker').attr("id") == $(e.currentTarget).attr("id");
                ...
                if (fixedStartDate > fixedEndDate) {
                    var $dateToBeChanged = isSelectingStartDate ? $('#endDatePicker') : $('#startDatePicker');
        ...
        disableFormOnSubmit: function(e){
            ...
            //show loading spinner and cancel link
            var self = this;
            setTimeout(function(){
                self.$element.find('.loadingRequestNotice').removeClass('hide');
            },500);

            //IE animated gif issue
            ...
            //register event for cancel link
...
FormElements = {
    ...
    _formElementsQuery: [ "toggle-switch", "datepickers" ],
    _componentsCollector: [],

    init: function () {
        for (var i = 0; i < this._formElementsQuery.length; i++) {
            FormElements.createComponent(this._formElementsQuery[i].replace("-", "_"));
        }
    },

    createComponent: function (ComponentName) {
        this._componentsCollector.push(ComponentName);
        this.Creator[ComponentName]();
    },
    ...
    Creator: {
        ...
        toggle_switch: function () {
            $(".toggle-switch").change(function() {
                if ( this.checked) {
                    $(this).closest(".toggle-container")
                        .find(".toggle-content")
                        .first()
                        .slideToggle()
                        .end()
                        .find("input[type=radio]").first().attr("checked", true);
                }
                else {
                    $(this).closest(".toggle-container")
                        .find(".toggle-content")
    ...
    Plugin.prototype = {
        ...
        validateForm: function() {
            ...
            if(!this.isValid) {
                ...
                //removing confirmation messages from previous successful actions,
                //to avoid confusion to the user (CBW/LB)
        ...
        preProcess: function(type, value) {
            // For telephone, remove whitespace within string
            if (type === "telephone") {
        ...
        // todo: Remove empty_or_numeric. Shouldn't have 2 rules in one
        ...
        markInvalid: function( $field, validate ) {
            ...
            $parentElement.find(".help-inline").html( $parentElement.find(".help-inline").data( validate + "Msg") );
...
;var Message = {
    ...
    MESSAGE_TOP: $(".navbar").size() > 0 ? $(".navbar").outerHeight() : 0,
    MESSAGE_TYPES: {
        "error": {
            "container_class": "msg newCustomMessages msg-error",
            "icon_class": "icon-exclamation-sign"
        },
        "info": {
            "container_class": "msg newCustomMessages",
            "icon_class": "sb-info-icon"
        },
        "confirmation": {
            "container_class": "msg newCustomMessages msg-confirm",
            "icon_class": "sb-confirmation-icon"
        }
    },
	...
	options:
	{
		html: '<div class="internal-message"><div class="@container_class"><p><i class="@icon_class"></i><span class="internal-error-message"></span></p><i class="message-close icon-delete"></i></div></div>',
        message_query: ".internal-error-message",
		container_query: '.container',
        close_query: ".message-close",
		type: "error",
        autoClose: false
	},
	...
	{
		var thisOption = $.extend ( {}, Message.options );
		if ( arguments.length > 0 )
			$.extend ( thisOption, arguments[1] );

        thisOption.html = thisOption.html.replace("@container_class", Message.MESSAGE_TYPES[thisOption.type].container_class);
        thisOption.html = thisOption.html.replace("@icon_class", Message.MESSAGE_TYPES[thisOption.type].icon_class);
        var newMessage = $( thisOption.html ).css ("z-index", Message.collector * 3 );

		newMessage.find ( thisOption.message_query ).html ( message );
		newMessage.find ( thisOption.close_query).click ( function () {
            Message._remove.call ( $(this).parents(".internal-message:first"))
        } );
        ...
        var MessageObject = {
            ...
            status: "hidden",
            element: newMessage,
            container: $(thisOption.container_query),
            message: message,
            show: function () {
                Message._show.call ( this );
                return this;
            },
            hide: function () {
                Message._hide.call ( this );
                return this;
            },
            edit: function ( text ) {
                Message._edit.call ( this, text );
                return this;
            }
        }
		Message.collector.push ( MessageObject );
        MessageObject.ref = Message.collector.length;

		return MessageObject
	},

    _hide: function ( ) {
        Message._updateStatus ( "hidden", this);
        this.element.animate ( { "top": (-this.element.outerHeight() ) + "px" });
    },

    _edit: function ( text ) {
        this.element.find ( this.options.message_query).html ( text );
    },

    _show: function () {

        if ( this.container.find ( ".internal-error" ).size () > 0 )
            this.container.find ( ".internal-error-message" ).append ( "<br />" + thisMessageObj.message );
        else
            this.container.append ( this.element );

        if ( this.status != "shown" ) {
            Message._updateStatus.call ( this, "shown" );
            this.element
                .css ( "top", ( -this.element.outerHeight() + Message.MESSAGE_TOP ) + "px" )
                .animate ( { "top": Message.MESSAGE_TOP } );
        }
//        for ( var i=0; i<Message.collector.length; i++) {
//            var item = Message.collector[i];
//            if ( this.ref != item.ref && this.status == "shown" ) {
//                item.element.animate ( { top: item.element.position().top + this.element..outerHeight() + Message.MESSAGE_TOP });
//            }
//        }
...
//var test = new Message.add ("Test messages on top").show ();
var SpeedboatUI = SpeedboatUI || {};
...
(function( $, context ){
    ...
    function Singleton(  ) {
        ...
        this.showStuckLoading = function() {
            this.template = $( templates.loading ).html();
            this.$element.empty().append( this.template );
            this.$element.modal({
                keyboard: false,
    ...
    // Expose object for test purposes. Never instantiate, use getInstance()
    ...
    Plugin.prototype = {
        ...
        triggerSorting: function(e){
            if($(e.currentTarget).closest('.notSortable').length > 0) {
                $(e.currentTarget).find('a').bind('click',function(){ return false; });
                return false;
            }
...
;(function ( $, context, document, undefined ) {
    ...
    Plugin.prototype = {
        ...
        setupEventHandlers: function() {
            ...
            this.$thePaginationTextfield.bind('focus', $.proxy(function(e){ this.paginationFieldWithFocusId = '#'+ e.currentTarget.id; },this));
            this.$thePaginationTextfield.bind('blur', $.proxy(function(e){ this.paginationFieldWithFocusId = null; },this));
...
//            else if((keyPressed==enter) && !($(e.currentTarget).closest('.control-group').hasClass('error'))){
//                //toggle disable button
//
//                var ex = jQuery.Event('click');
//                $(e.currentTarget).closest('form').find('#search').trigger(ex);
//                return false;
//            }
        },
        ...
        CheckForPaginationValues : function(e){
            ...
            if(this.paginationFieldWithFocusId != null){
                ...
                window.location = "//"+ window.location.host + $('input#dynamic_filter').val() +
                    "&ticketsPerPage=" + $('#ticketsPerPage').val() +
        ...
        RealtimeValidate: function(e){
            ...
            var $thePaginationTextfield = $(e.currentTarget); //couldn't work this out
...
;(function ( $, context, document, undefined ) {
    ...
    // Create the defaults once
    var pluginName = 'LiveCodeSourceDemo',
    defaults = {};
    ...
    Plugin.prototype = {
        ...
        init: function() {
            this.prepareDemoCode();

        },
        prepareDemoCode: function() {
        	var demoCode = this.$element.find('.demo').html();
            var demoCodeObj = $('<div class="demoCode"></div>');
            demoCodeObj.append ( $('<pre class="prettyprint"></pre>').text( demoCode ) );
            var demoCodeObjTools = $('<div class="demoCodeTools"></div>');
            this.$element.wrap ('<div class="pre-wrapper" />');
            this.$element.append( demoCodeObj );

            if ( demoCodeObj.find("pre").height() > this._maxCodeHeight ) {
                demoCodeObj.append ( demoCodeObjTools );
                demoCodeObj.find("pre").height (this._maxCodeHeight);
                demoCodeObjTools.append ( $('<span class="demoCodeToolsLabel">EXPAND</span>" ').click ( function () {
                    if ( $(this).text() == "EXPAND" ) {
                        $(this).text("COLLAPSE");
                        $(this).parents (".demoCode:first").find ("pre").height ("auto");
                    } else {
                        $(this).text("EXPAND");
                        $(this).parents (".demoCode:first").find ("pre").height (200);
                    }
                }));
            }
        },
...
//                                  debugger;
//            var backButtonPress = $(e.target).hasClass( this.options.classes.moveBackward )
//
//            if( this.isLastStep() ) {
//                return;
//            }
        ...
        step: function( direction, afterStepCallback ) {
            ...
            // Trigger event and exit if necessary
...
// Return the queryString. If there isn't one, returns an empty string
...
// Very simple method for getting key/value pairs from a string, e.g. a URL
...
Utils.disableOnClick = function(e){
    ...
    $cancelButton.bind('click', function(e) { return false; });
...
/*****************************************************
 * Config object is a store for application properties
 *****************************************************/

//#TODO Add string localisation

Speedboat.Config = {};
Speedboat.Config.applicationPath = "${pageContext.request.contextPath}/";
Speedboat.Config.basket = {
    getCountURL: Speedboat.Config.applicationPath + "procure/basket!total"
};

Speedboat.Config.confirmMessages = {
    confirmDeleteBasketItem: {
        msg: "basket.delete.msg.confirmation"
    },
    confirmDeleteCategoryBasketItems: {
        msg: "basket.delete.category.msg.confirmation"
    },
    confirmDeleteAllBasketItems: {
        msg: "basket.delete.all.msg.confirmation"
    },
    confirmTrashAsset: {
        msg: "manageresources.resources.msg.confirmation.remove"
    },
    confirmActivateAsset: {
        msg: "manageresources.resources.msg.confirmation.activate"
    },
    confirmDestroyAsset: {
        msg: "manageresources.resources.msg.confirmation.destroy"
    },
    confirmDeleteSnapshot: {
        msg: "manageresources.secondarystorage.msg.confirmation.delete.snapshot"
    },
    confirmRemoveTemplate: {
        msg: "manageresources.secondarystorage.msg.confirmation.remove.template"
    },
    confirmDeleteSecondaryStorage: {
        msg: "manageresources.secondarystorage.msg.confirmation.delete.storage"
    },
    forbiddenDeleteSecondaryStorage: {
        msg: "manageresources.secondarystorage.msg.forbidden.delete.storage"
    },
    confirmResetPassword: {
        msg: "usermanagement.messages.resetpassword.confirm"
    },
    confirmDeleteUser: {
        msg: "usermanagement.messages.deleteuser.confirm"
    },
    confirmLockUser: {
        msg: "usermanagement.messages.lockuser.confirm"
    },
    confirmUnlockUser: {
        msg: "usermanagement.messages.unlockuser.confirm"
    }
};
/* Configures underscore's template to use @ instead of } */
...
// Initialise the different patterns for showing and submitting forms
...
/***********************************************************************
 * Simple jQuery bits - no need to wrap up in a plugin or class
 ***********************************************************************/
...
(function() {
    ...
    // When the page loads, we don't want submit buttons to be disabled
    $forms.find("button[type=submit]").attr("disabled", null);
}());
    ...
    if (isRadioButtonChecked) {
        ...
        if (this.checked) {
            ...
            $container
                ...
                .slideUp(function() {
                    $container
                        .find(".toggle-content").find("input[type=radio]").attr("checked", false);
                })
        }
    });
...
// Only show modal dialogs when button is enabled
...
// Professional Services todo: generalise this
...
// CSS property table-layout: fixed; wraps long cells, however distributes columns evenly. This snippet
// switches on fixed layout after the dynamic widths have been set by the browser.
...
// asset manager, enable/disable CreateAndRun button
...
/*
 if($('.pagination')){
 new SpeedboatUI.PaginationManager({
...
$('table.sortableData').OrderingManager({ windowContext : window});


$('#ticketsPerPage').bind('change',function(e){   window.location = "//"+ window.location.host + $('input#dynamic_filter').val() +
    "&ticketsPerPage=" + $(e.currentTarget).val() +
    "&currentPageNumber=1";
});

$('textarea[maxlength]').bind('keydown keyup focus', Utils.restrictMaxLengthTextArea);
$('textarea[maxlength]').bind('paste',function(evt) { var theEvent = evt; setTimeout(function(){Utils.restrictMaxLengthTextArea(theEvent)}, 100); });
